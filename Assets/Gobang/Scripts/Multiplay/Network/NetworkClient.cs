...
public static class NetworkClient
{
    ...
    private static IEnumerator _Connect()
    {
        ...
        //异步连接
        IAsyncResult async = _client.BeginConnect(address, port, null, null);
        ...
        //设置退出事件
        NetworkCoroutine.Instance.SetQuitEvent(() => { _client.Close(); curState = ClientState.None; });
    }
    ...
    {
        //如果服务器下线, 客户端依然会继续发消息
        if (curState != ClientState.Connected || _stream == null)
        {
    ...
    public static void Connect(string address = null, int port = 8848)
    {
        ...
        //获取失败则取消连接
        if (!IPAddress.TryParse(address, out NetworkClient.address))
        {
